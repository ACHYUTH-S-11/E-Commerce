...
public interface CartClient {
    ...
    @GetMapping("/cart/user/{userId}")
    List<CartItemDTO> getCartItemsByUserId(@PathVariable("userId") Long userId); // Use @PathVariable instead of @RequestParam

    @DeleteMapping("/cart/clear/{userId}")
    void clearCart(@PathVariable("userId") Long userId); // Correct the endpoint to match the controller
}
